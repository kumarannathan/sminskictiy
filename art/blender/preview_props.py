...
kit.reset()
parts = json.load(open(os.path.join(kit.OUT_MESH, "props_parts.json")))
cols = json.load(open(os.path.join(kit.OUT_MESH, "props_colors.json")))
bpy.ops.import_scene.fbx(filepath=os.path.join(kit.OUT_MESH, "SminskiProps.fbx"))
...
for i, p in enumerate(props):
    lo, hi = bbox[p]
    k = 10 / max(hi - lo)
    origin = Vector(((i % 6) * 14 - 35, 0, -(i // 6) * 16))
...
kit.studio_lights(9)
kit.camera(rb((0, 38, 70)), rb((0, 4, -8)), lens=40)
kit.render(os.path.join(kit.ROOT, "preview", "props_preview.png"), 1280, 800, samples=64)
print("PP_OK")
